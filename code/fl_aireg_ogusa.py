# imports
import multiprocessing
from distributed import Client
import os, argparse
import json
import time
import importlib.resources
from importlib.resources import files
import copy
from pathlib import Path
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from taxcalc import Calculator
from ogusa.calibrate import Calibration
from ogcore.parameters import Specifications
from ogcore import output_tables as ot
from ogcore import output_plots as op
from ogcore.execute import runner
from ogcore.utils import safe_read_pickle
from ogcore.constants import (
    VAR_LABELS,
    ToGDP_LABELS,
)

# Use a custom matplotlib style file for plots
style_file_url = (
    "https://raw.githubusercontent.com/PSLmodels/OG-Core/"
    + "master/ogcore/OGcorePlots.mplstyle"
)
plt.style.use(style_file_url)

def main():
    # Define parameters to use for multiprocessing
    num_workers = min(multiprocessing.cpu_count() - 2, 10)
    client = Client(n_workers=num_workers, threads_per_worker=1)
    print("Number of workers = ", num_workers)

    # Directories to save data
    CUR_DIR = os.path.dirname(os.path.realpath(__file__))
    save_dir = os.path.join(CUR_DIR, "FL-AIregOutput")
    base_dir = os.path.join(save_dir, "OUTPUT_BASELINE")
    reform_dir_10 = os.path.join(save_dir, "OUTPUT_REFORM_10")
    figtab_dir_10 = os.path.join(reform_dir_10, "tables_figures")
    reform_dir_05 = os.path.join(save_dir, "OUTPUT_REFORM_05")
    figtab_dir_05 = os.path.join(reform_dir_05, "tables_figures")
    figdoc_dir = os.path.join(save_dir, "figs_for_doc")
    if not os.path.exists(figtab_dir_10):
        os.makedirs(figtab_dir_10)
    if not os.path.exists(figtab_dir_05):
        os.makedirs(figtab_dir_05)
    if not os.path.exists(figdoc_dir):
        os.makedirs(figdoc_dir)


    """
    ---------------------------------------------------------------------------
    Run baseline policy
    ---------------------------------------------------------------------------
    """
    # Set up baseline parameterization
    p = Specifications(
        baseline=True,
        num_workers=num_workers,
        baseline_dir=base_dir,
        output_base=base_dir,
    )
    # Update parameters for baseline from default json file
    with importlib.resources.open_text(
        "ogusa", "ogusa_default_parameters.json"
    ) as file:
        defaults = json.load(file)
    p.update_specifications(defaults)
    p.tax_func_type = "HSV"
    p.age_specific = True

    # c = Calibration(p, estimate_tax_functions=True, iit_baseline=iit_baseline, data='tmd', client=client)
    # c = Calibration(p, estimate_tax_functions=True, data='tmd', client=client)
    tmd_dir = "/Users/richardevans/Docs/Economics/OSE/microsim/tax-microdata-benchmarking/tmd/storage/output"
    c = Calibration(
        p,
        estimate_tax_functions=True,
        client=client,
        data=Path(os.path.join(tmd_dir, "tmd_jason2.csv.gz")),
        weights=Path(os.path.join(tmd_dir, "tmd_weights_jason2.csv.gz")),
        gfactors=Path(os.path.join(tmd_dir, "tmd_growfactors_jason2.csv")),
        records_start_year=2021,
    )
    client.close()
    d = c.get_dict()
    # # additional parameters to change
    updated_params = {
        "start_year": 2026,
        "RC_TPI": 100*1e-4,
        "inv_tax_credit": [[0.015]],
        "etr_params": d["etr_params"],
        "mtrx_params": d["mtrx_params"],
        "mtry_params": d["mtry_params"],
        "mean_income_data": d["mean_income_data"],
        "frac_tax_payroll": d["frac_tax_payroll"],
    }
    p.update_specifications(updated_params)

    # Run model
    start_time = time.time()
    client = Client(n_workers=num_workers, threads_per_worker=1)
    runner(p, time_path=True, client=client)
    print("run time = ", time.time() - start_time)
    client.close()

    """
    ---------------------------------------------------------------------------
    Run reform policy cut Z to 0.99 (by 1%) and cut labor productivity by 1%
    for the top earners down to 0.1% for the bottom earners
    ---------------------------------------------------------------------------
    """
    client = Client(n_workers=num_workers, threads_per_worker=1)
    # Set up baseline parameterization
    p2 = Specifications(
        baseline=False,
        num_workers=num_workers,
        baseline_dir=base_dir,
        output_base=reform_dir_10,
    )
    # Update parameters for baseline from default json file
    with importlib.resources.open_text(
        "ogusa", "ogusa_default_parameters.json"
    ) as file:
        defaults = json.load(file)
    p2.update_specifications(defaults)
    p2.tax_func_type = "HSV"
    p2.age_specific = True

    tmd_dir = "/Users/richardevans/Docs/Economics/OSE/microsim/tax-microdata-benchmarking/tmd/storage/output"
    c2 = Calibration(
        p2,
        estimate_tax_functions=True,
        client=client,
        data=Path(os.path.join(tmd_dir, "tmd_jason2.csv.gz")),
        weights=Path(os.path.join(tmd_dir, "tmd_weights_jason2.csv.gz")),
        gfactors=Path(os.path.join(tmd_dir, "tmd_growfactors_jason2.csv")),
        records_start_year=2021,
    )
    client.close()
    d2 = c2.get_dict()

    # Update the e matrix. Reduce the top productivity by 1%
    e_new = np.array(p2.e)
    pct_chg_vec = 1 - np.linspace(0.001, 0.010, e_new.shape[1])
    for col in range(e_new.shape[1]):
        e_new[:, col] = e_new[:, col] * pct_chg_vec[col]
    p2.e = e_new

    # additional parameters to change
    updated_params2 = {
        "start_year": 2026,
        "RC_TPI": 100*1e-4,
        "baseline_spending": True,
        "inv_tax_credit": [[0.015]],
        "Z": [[0.99]],
        "etr_params": d2["etr_params"],
        "mtrx_params": d2["mtrx_params"],
        "mtry_params": d2["mtry_params"],
        "mean_income_data": d2["mean_income_data"],
        "frac_tax_payroll": d2["frac_tax_payroll"],
    }
    p2.update_specifications(updated_params2)

    # Run model
    start_time = time.time()
    client = Client(n_workers=num_workers, threads_per_worker=1)
    runner(p2, time_path=True, client=client)
    print("run time = ", time.time() - start_time)
    client.close()

    """
    ---------------------------------------------------------------------------
    Run reform policy cut Z to 0.995 (by 0.5%) and cut labor productivity by
    0.5% for the top earners down to 0.05% for the bottom earners
    ---------------------------------------------------------------------------
    """
    # client = Client(n_workers=num_workers, threads_per_worker=1)
    # Set up baseline parameterization
    p3 = Specifications(
        baseline=False,
        num_workers=num_workers,
        baseline_dir=base_dir,
        output_base=reform_dir_05,
    )
    # Update parameters for baseline from default json file
    with importlib.resources.open_text(
        "ogusa", "ogusa_default_parameters.json"
    ) as file:
        defaults = json.load(file)
    p3.update_specifications(defaults)
    p3.tax_func_type = "HSV"
    p3.age_specific = True

    tmd_dir = "/Users/richardevans/Docs/Economics/OSE/microsim/tax-microdata-benchmarking/tmd/storage/output"
    c3 = Calibration(
        p3,
        estimate_tax_functions=True,
        client=client,
        data=Path(os.path.join(tmd_dir, "tmd_jason2.csv.gz")),
        weights=Path(os.path.join(tmd_dir, "tmd_weights_jason2.csv.gz")),
        gfactors=Path(os.path.join(tmd_dir, "tmd_growfactors_jason2.csv")),
        records_start_year=2021,
    )
    client.close()
    d3 = c3.get_dict()

    # Update the e matrix. Reduce the top productivity by 1%
    e_new = np.array(p3.e)
    pct_chg_vec = 1 - np.linspace(0.0005, 0.005, e_new.shape[1])
    for col in range(e_new.shape[1]):
        e_new[:, col] = e_new[:, col] * pct_chg_vec[col]
    p3.e = e_new

    # additional parameters to change
    updated_params3 = {
        "start_year": 2026,
        "RC_TPI": 100*1e-4,
        "baseline_spending": True,
        "inv_tax_credit": [[0.015]],
        "Z": [[0.995]],
        "etr_params": d3["etr_params"],
        "mtrx_params": d3["mtrx_params"],
        "mtry_params": d3["mtry_params"],
        "mean_income_data": d3["mean_income_data"],
        "frac_tax_payroll": d3["frac_tax_payroll"],
    }
    p3.update_specifications(updated_params3)

    # Run model
    start_time = time.time()
    client = Client(n_workers=num_workers, threads_per_worker=1)
    runner(p3, time_path=True, client=client)
    print("run time = ", time.time() - start_time)
    client.close()

    """
    ---------------------------------------------------------------------------
    Save some results of simulations
    ---------------------------------------------------------------------------
    """
    base_tpi = safe_read_pickle(os.path.join(base_dir, "TPI", "TPI_vars.pkl"))
    base_params = safe_read_pickle(os.path.join(base_dir, "model_params.pkl"))

    reform_tpi_10 = safe_read_pickle(
        os.path.join(reform_dir_10, "TPI", "TPI_vars.pkl")
    )
    reform_params_10 = safe_read_pickle(
        os.path.join(reform_dir_10, "model_params.pkl")
    )
    ans_10 = ot.macro_table(
        base_tpi,
        base_params,
        reform_tpi=reform_tpi_10,
        reform_params=reform_params_10,
        var_list=["Y", "C", "K", "L", "r", "w"],
        output_type="pct_diff",
        num_years=10,
        start_year=base_params.start_year,
    )

    # create plots of output
    op.plot_all(
        base_dir,
        reform_dir_10,
        os.path.join(figtab_dir_10),
    )
    # Create CSV file with output
    ot.time_series_table(
        base_params,
        base_tpi,
        reform_params_10,
        reform_tpi_10,
        table_format="csv",
        path=os.path.join(figtab_dir_10, "macro_time_series_output.csv"),
    )

    print("")
    print("Percentage changes in aggregates, 1.0% productivity decrease:")
    print(ans_10)
    # save percentage change output to csv file
    ans_10.to_csv(os.path.join(figtab_dir_10, "output.csv"))

    reform_tpi_05 = safe_read_pickle(
        os.path.join(reform_dir_05, "TPI", "TPI_vars.pkl")
    )
    reform_params_05 = safe_read_pickle(
        os.path.join(reform_dir_05, "model_params.pkl")
    )
    ans_05 = ot.macro_table(
        base_tpi,
        base_params,
        reform_tpi=reform_tpi_05,
        reform_params=reform_params_05,
        var_list=["Y", "C", "K", "L", "r", "w"],
        output_type="pct_diff",
        num_years=10,
        start_year=base_params.start_year,
    )

    # create plots of output
    op.plot_all(
        base_dir,
        reform_dir_05,
        os.path.join(figtab_dir_05),
    )
    # Create CSV file with output
    ot.time_series_table(
        base_params,
        base_tpi,
        reform_params_05,
        reform_tpi_05,
        table_format="csv",
        path=os.path.join(figtab_dir_05, "macro_time_series_output.csv"),
    )

    print("")
    print("Percentage changes in aggregates, 0.5% productivity decrease:")
    print(ans_05)
    # save percentage change output to csv file
    ans_05.to_csv(os.path.join(figtab_dir_05, "output.csv"))


if __name__ == "__main__":
    main()
